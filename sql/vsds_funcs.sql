...
CREATE OR REPLACE FUNCTION vsds.addsheetsurvey(cmdr text, campaign text) RETURNS int AS $$
DECLARE
	cmdrid int;
	campaignid int;
	mid int;
BEGIN
   ...
   SELECT INTO campaignid id FROM vsds.campaigns WHERE name = campaign;
   IF NOT FOUND THEN
      INSERT INTO vsds.campaigns (name) VALUES (campaign)
      RETURNING id INTO campaignid;
   END IF;

   INSERT INTO vsds.surveys (cmdrid, campaignid) VALUES (cmdrid, campaignid)
